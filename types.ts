...
export interface ConditionItem {
  id: string;
  field: string;       // 字段路径，如 fact.amount
  operator: ConditionOperator;
  value: string;       // 值（IS_NULL/IS_NOT_NULL 时为空）
}
...
export interface EventPoint {
  id?: number;
  eventPoint: string;        // 唯一编码，如 EP00000001
  description: string;
  status: FeatureStatus;      // 复用启用/禁用
  lifecycleState: FeatureLifecycle;
  createAt?: string;
  updateAt?: string;
  operator?: string;
}
...
export interface RiskThreshold {
  name: string;       // 档位名称，如 pass, review, reject
  score: number | null;  // 阈值分数，最后一档为 null
}
...
export interface Activation {
  ...
  name: string;
  description: string;
  eventPoint: string;          // 所属接入点编码
  status: FeatureStatus;
  lifecycleState: FeatureLifecycle;
  priority: number;
  thresholds: RiskThreshold[];  // 动态阈值配置
  createAt?: string;
  updateAt?: string;
  operator?: string;
}
...
export interface Action {
  ...
  type: ActionType;
  description: string;
  configSchema: string;        // JSON schema 定义
  status: FeatureStatus;
  lifecycleState: FeatureLifecycle;
  createAt?: string;
  updateAt?: string;
...
export interface Rule {
  ...
  name: string;
  description: string;
  activationName: string;      // 所属策略
  eventPoint: string;          // 所属接入点
  conditionExpression: ConditionExpression;
  initScore: number;
  baseNum: number;
...
export interface EventPointVersion {
  id: string;
  eventPointId: number;
  version: number;
  content: EventPoint;
...
export const t = (key: string | number): string => {
  const map: Record<string, string> = {
    ...
    [ConditionOperator.NOT_IN]: '不在列表中',
    ...
    // [ReleaseType.RULE]: '规则配置', // Duplicate key with ScopeLevel.RULE
    // [ReleaseType.EVENT_POINT]: '接入点配置', // Duplicate key with ScopeLevel.EVENT_POINT
    // [ReleaseType.ACTIVATION]: '策略配置', // Duplicate key with ScopeLevel.ACTIVATION
    [ReleaseType.ACTION]: '动作配置',

    [ReleaseStatus.PENDING]: '待审核',
    [ReleaseStatus.APPROVED]: '已审核',
    ...
    [RuleOperator.MUL]: '乘法',
    [RuleOperator.DIV]: '除法',
    [RuleOperator.NONE]: '无运算',

    // Action Types
    [ActionType.TAG]: '打标签',
    [ActionType.WEBHOOK]: 'Webhook回调',
    [ActionType.NOTIFY]: '通知',
    [ActionType.FEATURE_MUTATION]: '特征回写',

    // Execution Mode
    [ExecutionMode.SYNC]: '同步',
    [ExecutionMode.ASYNC]: '异步',
  };
