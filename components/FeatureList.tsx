...
export const FeatureList: React.FC<FeatureListProps> = ({ onAddToDrafts }) => {
  ...
  const eligibleCountPreview = selectedFeaturesList.filter(f => checkEligibility(f, batchAction).eligible).length;
                ...
                <tr key={feature.id} className={`hover:bg-[#f0f7ff] transition-colors group ${selectedIds.has(feature.id!) ? 'bg-indigo-50/30' : ''}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end gap-2">
                        ...
                        <button onClick={() => handleOpenEditor(feature)} className="flex items-center text-slate-500 hover:text-indigo-600 transition-colors" title="编辑特征"><Edit className="w-4 h-4 mr-1" />编辑</button>
